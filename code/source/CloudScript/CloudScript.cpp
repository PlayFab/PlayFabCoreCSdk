...
AsyncOp<GetFunctionResult> CloudScriptAPI::GetFunction(
    SharedPtr<Entity> entity,
...
)
{
    ...
    const char* path{ "/CloudScript/GetFunction" };
    JsonValue requestBody{ request.ToJson() };
    UnorderedMap<String, String> headers{{ kEntityTokenHeaderName, entityToken->token }};
    ...
    return requestOp.Then([](Result<ServiceResponse> result) -> Result<GetFunctionResult>
    {
        RETURN_IF_FAILED(result.hr);
        ...
        if (serviceResponse.HttpCode == 200)
        {
            GetFunctionResult resultModel;
            resultModel.FromJson(serviceResponse.Data);
            return resultModel;
        }
        else
        {
            return Result<GetFunctionResult>{ ServiceErrorToHR(serviceResponse.ErrorCode), std::move(serviceResponse.ErrorMessage) };
        }
    });
}
