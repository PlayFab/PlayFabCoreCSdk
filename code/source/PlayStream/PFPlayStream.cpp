...
) noexcept
{
    ...
    auto provider = MakeProvider(async, __FUNCTION__, std::bind(&PlayStreamAPI::AdminExportPlayersInSegment, contextHandle->state, *request, std::placeholders::_1));
...
) noexcept
{
    ...
    auto provider = MakeProvider(async, __FUNCTION__, std::bind(&PlayStreamAPI::AdminGetSegmentExport, contextHandle->state, *request, std::placeholders::_1));
    return Provider::Run(UniquePtr<Provider>(provider.release()));
}
